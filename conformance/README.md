...
| DataRef / 行级 Action / 行级 navigate / recordSource / 页面 Trigger（request·navigate·modal·confirm）/ batch 请求构造 | `fixtures/request-construction/cases.json` | `reference-js/request-construction.js`、`reference-python/request_construction.py` | `npm run test:conformance:request`、`npm run test:conformance:request:python` |
...
| 应用导航（ADR-0026） | `fixtures/app-navigation/cases.json`（含 M3a 表达式静态校验反例） | `reference-js/app-navigation.js`、`reference-python/app_navigation.py` | `npm run test:conformance:app-navigation`、`npm run test:conformance:app-navigation:python` |
...
- **算法类 suite**（request / response / component-format / search-table / reactions / request-lifecycle / runtime-defaults / static-data / actions / permissions-inheritance / uploads / scenarios / host-bootstrap / host-failure / host-conformance-claim / app-manifest / app-navigation / table-sort）：`protocolVersion` 表示**该 case 适用的协议算法版本**（当前制品 MAJOR.MINOR 为 `"2.9"`），不是历史包版本号。消费者可按上一线（如 `"2.7"`）过滤跑互操作向量。
...
- `release:check` 与 `validate:conformance` 对非 version-negotiation suite 强制 case.`protocolVersion` 等于当前制品 MAJOR.MINOR（由根包 SemVer 推导，现为 `"2.9"`）。
