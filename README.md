...
当前协议版本为 `2.8.2`，页面声明使用 `meta.protocolVersion: "2.8"`。
...
| 层级 | 内容 | 作用 |
|---|---|---|
| 核心规范 | `docs/00`–`10`、协议 ADR | 定义字段语义、默认值和能力边界 |
...
```text
dist/schema-ui-protocol-2.8.2.tar.gz
dist/schema-ui-protocol-2.8.2.tar.gz.sha256
...
`2.8` 请按 [`docs/migrations/2.7-to-2.8.md`](./docs/migrations/2.7-to-2.8.md) 执行；从 `2.6` 请先按
...
- 协议 tag：`v<protocol-artifact-version>`，例如 `v2.8.2`；
- 页面协议版本：MAJOR.MINOR，例如 `2.8`；
- MCP tag：`mcp-v<mcp-version>`，例如 `mcp-v2.8.2`（tag 独立推送；**MAJOR.MINOR 与协议线对齐**，PATCH 可独立）；
...
`scripts/` 提供 L0/L1、L2、L3a、L4 校验实现。当前 validator 版本为 `1.0.0`，支持协议制品 `2.8.2`，
...
也不进入协议权威层。MCP 当前版本为 `2.8.2`，捆绑协议制品 `2.8.2`（MAJOR.MINOR 共线；MCP PATCH 可独立演进）。正式分发为
...
当前发布门禁见 [`docs/release-goals/v2.8.md`](./docs/release-goals/v2.8.md)（目录说明见
