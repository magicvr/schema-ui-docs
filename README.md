...
当前协议版本为 `2.5.2`，页面声明使用 `meta.protocolVersion: "2.5"`。
...
```text
dist/schema-ui-protocol-2.5.2.tar.gz
dist/schema-ui-protocol-2.5.2.tar.gz.sha256
...
- 协议 tag：`v<protocol-artifact-version>`，例如 `v2.5.2`；
- 页面协议版本：MAJOR.MINOR，例如 `2.4`；
- MCP tag：`mcp-v<mcp-version>`，例如 `mcp-v2.5.2`（tag 独立推送；**MAJOR.MINOR 与协议线对齐**，PATCH 可独立）；
...
`scripts/` 提供 L0/L1、L2、L3a、L4 校验实现。当前 validator 版本为 `1.0.0`，支持协议制品 `2.5.2`，
...
也不进入协议权威层。MCP 当前版本为 `2.5.2`，捆绑协议制品 `2.5.2`（MAJOR.MINOR 共线；MCP PATCH 可独立演进）。正式分发为
