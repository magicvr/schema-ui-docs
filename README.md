...
当前跨实现消费规则与 fixture 分类见 [`conformance/README.md`](./conformance/README.md)。从 `2.6` 升级到
`2.7` 请按 [`docs/migrations/2.6-to-2.7.md`](./docs/migrations/2.6-to-2.7.md) 执行；从 `2.5` 请先按
...
- 协议 tag：`v<protocol-artifact-version>`，例如 `v2.7.0`；
- 页面协议版本：MAJOR.MINOR，例如 `2.7`；
- MCP tag：`mcp-v<mcp-version>`，例如 `mcp-v2.7.0`（tag 独立推送；**MAJOR.MINOR 与协议线对齐**，PATCH 可独立）；
...
`scripts/` 提供 L0/L1、L2、L3a、L4 校验实现。当前 validator 版本为 `1.0.0`，支持协议制品 `2.7.0`，
...
也不进入协议权威层。MCP 当前版本为 `2.7.0`，捆绑协议制品 `2.7.0`（MAJOR.MINOR 共线；MCP PATCH 可独立演进）。正式分发为
...
当前发布门禁见 [`docs/release-goals/v2.7.md`](./docs/release-goals/v2.7.md)（目录说明见
