...
当前协议版本为 `2.2.0`，页面声明使用 `meta.protocolVersion: "2.2"`。
...
```text
dist/schema-ui-protocol-2.2.0.tar.gz
dist/schema-ui-protocol-2.2.0.tar.gz.sha256
...
当前跨实现消费规则与 fixture 分类见 [`conformance/README.md`](./conformance/README.md)。从 `2.1` 升级到
`2.2` 请按 [`docs/migrations/2.1-to-2.2.md`](./docs/migrations/2.1-to-2.2.md) 执行；从 `2.0` 请先按
...
- 协议 tag：`v<protocol-artifact-version>`，例如 `v2.2.0`；
- 页面协议版本：MAJOR.MINOR，例如 `2.2`；
...
`scripts/` 提供 L0/L1、L2、L3a、L4 校验实现。当前 validator 版本为 `1.0.0`，支持协议制品 `2.2.0`，
...
也不进入协议权威层。MCP 当前版本为 `2.0.0`，捆绑协议制品 `2.2.0`；两个版本独立演进。正式分发为
...
协议范围与当前发布门禁见 [`docs/13-v2.2-release-goals.md`](./docs/13-v2.2-release-goals.md)；
