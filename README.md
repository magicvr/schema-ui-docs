...
当前协议版本为 `2.3.0`，页面声明使用 `meta.protocolVersion: "2.3"`。
...
```text
dist/schema-ui-protocol-2.3.0.tar.gz
dist/schema-ui-protocol-2.3.0.tar.gz.sha256
...
- 协议 tag：`v<protocol-artifact-version>`，例如 `v2.3.0`；
...
`scripts/` 提供 L0/L1、L2、L3a、L4 校验实现。当前 validator 版本为 `1.0.0`，支持协议制品 `2.3.0`，
...
也不进入协议权威层。MCP 当前版本为 `2.0.0`，捆绑协议制品 `2.3.0`；两个版本独立演进。正式分发为
