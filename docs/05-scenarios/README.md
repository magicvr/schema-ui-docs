...
> `admin-list-batch.md` 为 Phase C 扩展示例：机器可读样例见 `_samples/order-list-batch.yaml`；当前示例随协议线统一声明 `protocolVersion: "2.6"`（batch 字段集仍属 2.2+ capability）；**已进入** `release-check` 场景 frontmatter 与 YAML `meta.protocolVersion` 一致性门禁。
