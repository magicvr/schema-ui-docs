...
本示例拆成**两个页面文档**（列表 + 编辑），覆盖轨道 MVP 锚点 A/B/C。使用前 Renderer 须支持相应 `requiredCapabilities`。页面声明 `meta.protocolVersion: "2.7"` 与对应 capability；`2.1.0` 制品已发布，不再建议用 `"2.0"` 承载本场景字段。
