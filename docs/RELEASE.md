...
| Tag 模式 | 版本来源 | 工作流 | 产物 |
|---|---|---|---|
| `v<artifactVersion>` 如 `v2.1.0` | 根 `package.json` / `protocol-manifest.json` | [protocol-release.yml](../.github/workflows/protocol-release.yml) | GitHub Release：`schema-ui-protocol-*.tar.gz` + `.sha256` |
| `mcp-v<mcpVersion>` 如 `mcp-v2.0.0` | `mcp/package.json` | [mcp-cd.yml](../.github/workflows/mcp-cd.yml) | GHCR：`ghcr.io/<owner>/schema-ui-mcp` |

- 两套 tag **必须独立**；协议 MINOR 发布不强制同日发 MCP。
- 发 `mcp-v*` 前，MCP 的 `schemaUiProtocol.artifactVersion` 应指向**已发布**的协议制品版本。
- 预发布：协议可用 `v2.1.0-rc.1` 等（以 `release:check:tag` 规则为准）；MCP 预发布 tag 只推完整版本与 commit SHA 镜像，**不**更新 `latest` / minor 别名。
...
5. 消费者 pin：`v2.1.0` 或 tar 的 SHA-256。
...
拉取与运行示例见 [mcp/README.md](./mcp/README.md)。
...
| 文件 | 触发 |
|---|---|
| [protocol-ci.yml](../.github/workflows/protocol-ci.yml) | PR / push `main`（协议路径） |
| [mcp-ci.yml](../.github/workflows/mcp-ci.yml) | PR / push `main`（MCP 路径） |
| [validator-ci.yml](../.github/workflows/validator-ci.yml) | 校验器相关变更 |
| [protocol-release.yml](../.github/workflows/protocol-release.yml) | tag `v*` |
| [mcp-cd.yml](../.github/workflows/mcp-cd.yml) | tag `mcp-v*` |
