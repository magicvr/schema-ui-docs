...
Renderer 维护一个核心组件注册表，将 `type` 字符串映射到协议组件实现。核心注册表只包含 `component-registry.json` 声明的类型。

宿主可以通过私有 API（例如 `Renderer.register()`）安装 Host Extension，但该 API 不属于 v2.0 页面协议，扩展页面不得以核心协议身份跨 Renderer 互操作。扩展必须由宿主显式安装；未安装的扩展 type 在标准入口以 `UNKNOWN_COMPONENT_TYPE` 拒绝，不得静默降级为核心组件。

- schema `default` 只描述协议默认值，Renderer 执行时物化；缺少必填 props 或未安装扩展 type 时进入结构化 invalid-component/unknown-component 状态，不猜测业务默认值。
- 静态 `source: static` 和静态 `source: ref` 同步消费 `value`，不产生网络 loading/error；请求型页面必须提供非空 baseURL，否则返回 `MISSING_BASE_URL`。
...
- 缺失必填 `props` 字段必须进入结构化 `INVALID_COMPONENT` 状态；Renderer 不得猜测业务默认值继续执行。
...
- 每个数据消费 Node 维护单调递增的 request generation；只有当前 generation 且 Node 仍 mounted/active 的响应才能提交数据、loading 结束态或错误态。旧响应必须丢弃，取消请求仅为尽力优化。
...
`requestInterceptor` 只允许修改请求 URL、header、credentials、body 等传输层信息，不应改写 Node 配置或组件状态。DataRef/optionsSource 的 GET 请求经过拦截器后仍不得带 body；拦截器不得改变协议已确定的 method 或 query 语义。`401`/`403` 的协议级错误态优先于任何 `states.error.fallbackText`；401 不展示具体响应文案，403 使用固定无权限占位。
...
| `app.manifest` | 应用级清单获取、页面注册表与 home 落地 | ADR-0025 / [17](./17-app-manifest.md) |
| `app.navigation` | 清单 `navigation` 槽位/分组/过滤/高亮 | ADR-0026 / [17](./17-app-manifest.md) |
...
1. `401`/`403` 的协议级错误态优先于任何 `states.error.fallbackText`；401 不展示具体响应文案，403 使用固定无权限占位。
2. 其他失败 Node 若定义了 `states.error.fallbackText`，展示该文案。
3. 若未定义 `states.error`，Renderer 使用默认错误提示（如"渲染失败"图标 + 文案）。
...
Renderer mount 时必须执行一次初始 Snapshot/Evaluate/Commit。每个受 reaction 控制的状态键保存组件初始基线；条件变为 false 且没有 `otherwise` 时，该键恢复基线。`otherwise` 存在时使用其显式值。基线不会被后续 reaction 提交覆盖。
...
- Renderer 在初始化时必须接收非空 `baseURL` 参数；包含网络 DataRef、optionsSource、Action、UploadAction 或 navigate URL 的页面缺少 baseURL 时，必须在发出请求前返回 `MISSING_BASE_URL`，不得猜测 same-origin fallback。Renderer 在初始化时接收 `baseURL` 参数，自动拼接：
