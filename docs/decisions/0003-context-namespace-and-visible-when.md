...
> **0039 / V122 澄清：** 保留 D1 的实例初始化快照口径。`$context` 不在同一 Renderer 实例内响应式刷新；宿主更新 context 时必须重挂载并创建新实例，新实例首次求值读取新快照。
...
- 节点所处的表单上下文由 Node 树位置决定(节点位于 `type: form` 子树中即为表单上下文)。非表单上下文(例如布局容器 `section`/`grid`)中,`visibleWhen` 的 `dependencies` 可省略,此时 `when` 中只允许使用 `$context.*` 变量,不允许出现 `$deps`。注意:表单上下文内缺失 `dependencies` 是配置错误,不会因此变成非表单上下文。
- **静态校验规则(补充)**:若节点处于非表单上下文但其 `when` 表达式中出现了 `$deps.*`,协议要求解析器在**静态校验阶段直接拒绝**该协议(视为格式错误),而不是运行时静默返回 `undefined` 或视为合法但取不到值。理由:静默容错在这里反而有害——`$deps` 出现在不该出现的位置几乎总是配置错误(拼写误用、复制粘贴遗留),直接拒绝能在问题发生的第一时间暴露,而不是让开发者花时间排查"为什么这个条件永远不满足"。
...
- `$context.user` 自 v0.2.5 起已有协议级最小字段集(`id`/`name`/`roles`),跨项目至少可复用基于该最小集的权限表达式;但项目扩展字段仍可能命名不一致,跨项目复用依赖扩展字段的表达式时仍需要额外映射层——这是有意识的取舍(避免协议层过度设计完整业务身份模型),最小集见 D1 与 [`02-reaction-expression.md` §11.1](../02-reaction-expression.md#111-contextuser-最小字段集)。
...
- **三路 AND 公式(D3d)内部的求值时序问题已由 ADR-0006 解决**:若某个字段同时被 `visibleWhen` 引用,又被同节点或其他节点的 `reactions.fulfill.value` 修改,按 ADR-0006 的稳定快照模型处理——本轮读取求值开始时的旧快照,本轮末尾批量提交写入,下一轮求值才读取新值。
