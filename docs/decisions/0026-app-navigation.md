...
- **面包屑**：非应用互操作必需；静态段多可由注册表 + 当前路由推导，动态末级依赖记录数据，
  另议；本 ADR **不做**。
- **菜单 badge / 红点 / 计数**：活数据，与清单一次性快照冲突；刷新、权限与形态不统一，
  属运营增强，**不做**（宿主可按 `pageId` 私有扩展，不得写入核心正例）。
...
- 默认落地页（在 [ADR-0025](./0025-app-manifest.md) D3a `homePageRef`，不在导航结构内）。
