...
**Accepted（已接受）。** 字段与执行语义以本 ADR 及同步更新的 `02` / `03` / `07` / `08` / Schema / L2 为准。使用时分别声明 `actions.row.navigate` 与/或 `form.record.load`。制品打包目标与 [ADR-0020](./0020-page-action-trigger.md) 同属 **MINOR `2.1`**；在 `protocolVersion: "2.1"` 正式发布前，合法页面可继续声明 `protocolVersion: "2.0"` 并依赖 capability 协商。开放问题裁决见文末，已并入 D1 / D4–D7。
