...
L2/L3a/L4 子脚本在输出结构化 JSON 后必须通过 `process.exitCode` 自然结束，让 stdout 完整冲刷；不得在大 payload 后立即 `process.exit(1)`。校验适配器通过 `spawnSync` 调用子脚本并使用显式 16MB stdout 上限，协议违规的预期 exit code 1 仍解析完整 stdout。若超过该上限，必须返回结构化 `internalError`，不得把截断输出归类为 JSON 解析失败。
...
实现可以通过临时文件或内存适配复用同一组校验规则；对外语义必须等价于同版本 CI 校验链，但 MCP 内部不强制调用统一入口：

```bash
node scripts/validate-all.js <page-file>
```

`scripts/validate-all.js` 保留为 CI 与人工本地校验入口。MCP 可以直接使用 AJV API 执行 L0/L1，并调用 L2/L3a/L4 子脚本的 `--json` 输出，以避免解析聚合后的 stdout/stderr 文本。
...
- Docker 镜像必须预装 MCP 运行时依赖与执行 L0/L1 所需的 AJV 运行库，不得依赖宿主环境提供校验依赖；
- 临时文件扩展名必须与 `format` 保持一致（`.yaml` / `.json`），使现有脚本的解析路径与 CI 一致。

### D6. 技术栈

MCP v1 使用 Node.js + TypeScript 实现，源码安装、校验工具链与运行时要求 Node.js >= 20.19.0。
...
- 现有依赖（`js-yaml`、`glob`、AJV 生态包）可直接复用；
- TypeScript 生态中 MCP SDK 支持成熟；
- 当前 `vitest` 测试链间接使用 Vite 7，其 Node 支持范围从 20.19.0 起；将公开下限统一到可由标准安装、构建和测试路径持续验证的版本，CI 同时覆盖最低版本与 Node 24；
...
单次工具返回的最终文本不超过 20KB；预算包含 JSON 元数据、正文、截断提示、序列化转义/缩进和章节列表，而不是只计算 `content` 字段。裁剪必须保持 UTF-8 字节边界安全。`protocol.validate_content` 不得截断 JSON 字符串，而应按层裁剪违规数组，并通过 `layerStats`（`total` / `returned` / `omitted`）和顶层 `truncated` 暴露省略情况；每个存在违规的层至少保留一条代表项。若未来文档规模扩大，需要分页或索引检索时另行扩展。
...
语法合法但根值不是页面对象（例如 JSON `null`、字符串、数组或空 YAML 文档）属于 L0/L1 结构校验失败，不属于输入解析失败：`parseError` 必须为 `null`，L0/L1 返回根类型违规，L2-L4 保持为空。
