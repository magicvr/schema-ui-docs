...
| 项 | 决策 |
|---|---|
| 技术栈 | Node.js >= 20.19.0 + TypeScript |
...
最低版本 `20.19.0` 与仓库标准测试链保持一致：当前 `vitest` 间接使用 Vite 7，而 Vite 7 的 Node 支持范围从 `20.19.0` 起。CI 在该最低版本与 Node 24 上执行 MCP 安装、构建、测试和 tools smoke；官方 Docker 镜像仅使用 Node 24。
...
- 完整 MCP 工具文本（包含 JSON 元数据、正文、截断提示、转义/缩进和 `availableSections`）不超过 20KB；超出时按 UTF-8 边界裁剪正文，并附章节标题数组，便于调用方使用更精确的 `section` 重试。
...
```json
{
  ...
  "layerStats": {
    "L0/L1": { "total": 0, "returned": 0, "omitted": 0 },
    "L2": { "total": 0, "returned": 0, "omitted": 0 },
    "L3a": { "total": 1, "returned": 1, "omitted": 0 },
    "L4": { "total": 0, "returned": 0, "omitted": 0 }
  },
  "truncated": false,
...
`protocol.validate_content` 与其他工具一样受最终工具文本 20KB 预算约束。违规数量较多时，服务按层保留可容纳的前序违规，并通过 `layerStats.total/returned/omitted` 与顶层 `truncated` 标明省略数量；不得直接截断 JSON 字符串。每个存在违规的层至少保留一条代表项。语法合法但根值不是页面对象（如 JSON `null`、数组或空 YAML）属于 L0/L1 结构失败，`parseError` 保持 `null`，并跳过不适用的 L2-L4。页面对象内部的类型错误（如 `body.props: null`）同样属于结构违规：L0/L1 报告具体路径，L2-L4 跳过不适用的语义检查，不得把脚本类型异常提升为 `parseError`。

L2/L3a/L4 子脚本输出结构化 JSON 后通过 `process.exitCode` 设置结果码，让事件循环自然冲刷 stdout；禁止在大 payload 后立即 `process.exit(1)`，后者会在 Linux 管道中截断尚未写完的 JSON。适配器通过 `spawnSync` 执行子脚本并使用显式 16MB stdout 内部上限，协议违规导致的预期 exit code 1 仍直接解析完整 stdout。若单层输出超过该上限，返回对应层的结构化 `internalError`“校验脚本输出超过 16MB 内部上限”，不得误报为 JSON 解析失败。最终工具文本仍独立遵守 20KB 预算。
...
2. 先用 `js-yaml` 或 `JSON.parse` 做输入解析，解析失败直接返回 `parseError`；语法合法的非对象根值只进入 L0/L1；
...
6. 分别调用 `validate-l2-components.js`、`validate-l3a-expressions.js`、`lint-l4-banned-props.js` 的 `--json` 输出；子脚本以 `process.exitCode` 自然结束，父进程使用显式 16MB stdout 上限；
