...
MCP v1 复用现有 `scripts/validate-all.js`，不重写 L0-L4 校验规则。
...
5. 调用 `node scripts/validate-all.js <tmp-file>`；
6. 解析各层输出为结构化 `layers`；
7. 按固定映射生成 `suggestedDocs`；
8. 清理临时目录。

临时目录清理失败时，只输出本地警告，不改变校验结果。Docker 镜像必须内置 `ajv-cli` 与 `ajv-formats`，确保 L0/L1 可运行。

MCP 不得直接透传 `validate-all.js --json` 的输出。当前统一入口的 JSON 结果包含各层 `stdout` / `stderr` 原文，而 MCP 需要的是按层归一化后的违规数组。因此实现必须先确认各子脚本的 `--json` 输出 schema，再建立转换映射：
...
| `ajv-cli` L0/L1 | AJV errors → `layers["L0/L1"]`，转换 `instancePath` / `keyword` / `message` |
