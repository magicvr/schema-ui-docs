...
L2/L3a/L4 子脚本 stdout 使用显式 16MB 内部上限，避免 Node 默认缓冲区在远低于 1MB 输入上限时截断真实违规 JSON。若单层输出仍超过该上限，返回对应层的结构化 `internalError`“校验脚本输出超过 16MB 内部上限”，不得误报为 JSON 解析失败。最终工具文本仍独立遵守 20KB 预算。
...
6. 分别调用 `validate-l2-components.js`、`validate-l3a-expressions.js`、`lint-l4-banned-props.js` 的 `--json` 输出，并使用显式 16MB stdout 上限；
