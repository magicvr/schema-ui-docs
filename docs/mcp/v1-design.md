...
`protocol.validate_content` 与其他工具一样受最终工具文本 20KB 预算约束。违规数量较多时，服务按层保留可容纳的前序违规，并通过 `layerStats.total/returned/omitted` 与顶层 `truncated` 标明省略数量；不得直接截断 JSON 字符串。每个存在违规的层至少保留一条代表项。语法合法但根值不是页面对象（如 JSON `null`、数组或空 YAML）属于 L0/L1 结构失败，`parseError` 保持 `null`，并跳过不适用的 L2-L4。
