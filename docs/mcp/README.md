...
当前版本示例（版本号以 `mcp/package.json` 与**已发布** `mcp-v*` tag 为准；应与协议线一致，例如协议 `2.7` → MCP `2.7.x`。工作树 MCP 为 `2.7.0`（捆绑协议 `2.7.0`）；**正式** `mcp-v2.7.0` 发布前生产可继续 pin 已发布的 **`2.6.0`** / **`mcp-v2.6.0`**，发版后请 pin **`2.7.0`** / **`mcp-v2.7.0`**，不要只用 `latest`）：

```bash
docker pull ghcr.io/<owner>/schema-ui-mcp:2.7.0
docker run --rm -i ghcr.io/<owner>/schema-ui-mcp:2.7.0
```
  ...
  "mcpServers": {
    "schema-ui": {
      ...
      "args": [
        ...
        "--rm",
        "-i",
        "ghcr.io/<owner>/schema-ui-mcp:2.7.0"
      ]
    }
  }
}
...
| Tag | 用途 |
|---|---|
| `2.7.0` | 固定版本（工作树 / 发版后稳定接入示例；MAJOR.MINOR = 协议线） |
| `2.7` | 当前 `2.7.x` 最新稳定 PATCH（与协议 `2.7` 线对应；随稳定 `mcp-v*` 更新） |
...
```bash
docker build -f mcp/Dockerfile -t schema-ui-mcp:2.7.0 .
```

作为 stdio MCP server 启动：

```bash
docker run --rm -i schema-ui-mcp:2.7.0
```

Docker smoke test：

```bash
npm --prefix mcp run smoke:docker -- schema-ui-mcp:2.7.0
```
