...
当前版本示例（版本号以已发布 MCP tag 为准；应与协议线一致，例如协议 `2.5` → MCP `2.5.x`。工作树 MCP 为 `2.5.0`，正式拉取需等 `mcp-v2.5.0` 发布）：

```bash
docker pull ghcr.io/<owner>/schema-ui-mcp:2.5.0
docker run --rm -i ghcr.io/<owner>/schema-ui-mcp:2.5.0
```
  ...
  "mcpServers": {
    "schema-ui": {
      ...
      "args": [
        ...
        "--rm",
        "-i",
        "ghcr.io/<owner>/schema-ui-mcp:2.5.0"
      ]
    }
  }
}
...
| Tag | 用途 |
|---|---|
| `2.5.0` | 固定 PATCH 版本（稳定接入示例；MAJOR.MINOR = 协议线） |
...
```bash
docker build -f mcp/Dockerfile -t schema-ui-mcp:2.5.0 .
```

作为 stdio MCP server 启动：

```bash
docker run --rm -i schema-ui-mcp:2.5.0
```

Docker smoke test：

```bash
npm --prefix mcp run smoke:docker -- schema-ui-mcp:2.5.0
```
