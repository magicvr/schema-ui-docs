...
> 当前工作区 MCP 包版本为 `0.3.0-rc.1`，已发布稳定 Docker 镜像示例仍为 `0.2.8`。候选镜像只有在完成构建、Docker smoke 和推送后才能声明已发布；版本状态以 [`docs/CHANGELOG.md`](../CHANGELOG.md) 为准。
...
CD 仅接受与 `mcp/package.json` 版本完全一致的 `v<version>` Git tag。预发布版本（如 `v0.3.0-rc.1`）只生成完整版本 tag 与 commit SHA，不更新 minor 或 `latest`；无预发布标识的稳定版本才更新这两个别名。`workflow_dispatch` 也必须从匹配的 Git tag 触发，不能从普通分支覆盖已发布版本。
...
已发布镜像接入示例固定使用稳定 tag `0.2.8`，本地候选构建固定使用 `0.3.0-rc.1`，均不使用 `latest`。`mcp/package.json` 中的 MCP SDK 依赖也应固定为明确版本，而不是使用 `latest` 作为包清单策略，避免后续刷新 lockfile 时无意引入 SDK 行为漂移。
