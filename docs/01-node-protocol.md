...
meta:            # 页面元信息
  ...
  title: string
  description: string
  protocolVersion: string   # 必填（since 0.2）。当前制品使用 "2.8"；字段集→版本下限见下文与 migrations/
...
> **`meta.protocolVersion`（since 0.2）：** 必填字符串，格式为 MAJOR.MINOR（当前协议版本使用 `"2.8"`），**不含 PATCH 或预发布标识**。Renderer 据此判断按哪套解析规则处理页面文档，是协议后续演进做版本兼容判断的锚点。同一 MAJOR.MINOR 的补丁或 RC 包共享该值；Renderer 的兼容性判断基于支持的 MAJOR.MINOR，不依赖包 PATCH。旧 `v0.1` 文档缺少该字段时仅可由显式 legacy adapter 处理；v1.0 文档必须先经过显式迁移 adapter；新建或修改的文档必须显式声明。仅使用 2.1 字段集的页面可继续声明 `"2.1"`；使用 `table.selection` / `batchMapping` / `requiresSelection` 时必须至少 `"2.2"`；使用 `permissionCascade` 或 `permissionIntent` 时必须 `"2.3"`（L2 fail-closed）；使用 `type: recordView` 时必须 `meta.protocolVersion >= "2.4"` 且声明 `meta.requiredCapabilities` 含 `record.view.load`（L2 双重门控 fail-closed，ADR-0024）；使用 `sortable` / `sortField` / `defaultSort` 时必须 `meta.protocolVersion >= "2.5"` 且声明 `table.sort`（ADR-0027）；使用 `textarea` / `switch` / `checkbox` / `radio` 或 `select.mode: multiple` 时必须 `meta.protocolVersion >= "2.6"` 且声明 `form.controls.extended`（ADR-0028）；使用 `cascader` / `checkboxGroup` / `richText` / `password` 或 `props.defaultValue` 时必须 `meta.protocolVersion >= "2.7"` 且声明 `form.controls.advanced`（ADR-0029–0033）。字段集→版本下限由 L2 强制，见 [06-validation.md](./06-validation.md) 与 [00-overview.md](./00-overview.md)。

> **`meta.requiredCapabilities`（可选，since 0.2.6）：** 字符串数组，声明页面依赖的 Renderer 执行能力，用于补足 PATCH 级能力协商。`protocolVersion` 仍只表达结构版本；当 PATCH 版本新增需要 Renderer 执行支持的能力时，页面必须通过 `requiredCapabilities` 显式声明。Renderer 若不支持其中任一能力，应在静态校验阶段拒绝渲染，而不是进入运行时后部分失效。当前协议预定义能力键：`actions.upload`、`actions.row.request`、`actions.page.trigger`、`actions.row.navigate`、`form.record.load`、`table.selection`、`actions.batch.request`、`permissions.inheritance`（ADR-0023）、`record.view.load`（ADR-0024）、`app.manifest`（ADR-0025）、`app.navigation`（ADR-0026）、`table.sort`（ADR-0027）、`form.controls.extended`（ADR-0028）、`form.controls.advanced`（ADR-0029–0033）。完整表见 [08-renderer-spec.md §3.4](./08-renderer-spec.md#34-执行能力匹配规则since-026)。
...
|---|---|
| `view` | 节点不渲染且不参与交互（用户无权限） |
| `edit` | 节点以只读或禁用形态呈现，不提供编辑/操作入口 |
| `delete` | 对应操作按钮禁用/隐藏 |
