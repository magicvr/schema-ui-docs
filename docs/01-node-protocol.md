...
meta:            # 页面元信息
  ...
  title: string
  description: string
  protocolVersion: string   # 必填（since 0.2）。当前制品使用 "2.5"；字段集→版本下限见下文与 migrations/
  requiredCapabilities: [string] # 可选（since 0.2.6）。执行能力协商，如 actions.page.trigger / table.selection / permissions.inheritance / record.view.load / table.sort / form.controls.extended / app.manifest
