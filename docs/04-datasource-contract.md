...
`source: api` 时 `method` 可选，**缺省为 `GET`**。DataRef 是只读数据加载契约；显式声明 `POST`、`PUT`、`PATCH` 或 `DELETE` 的 DataRef 必须被 Schema/L2 拒绝，写操作使用 Action。

## 3. 通用请求约定

`data.params` 只为 DataRef 的 `GET` 请求编码为 URL query 参数。DataRef 不定义请求体字段；需要 JSON 请求体或其他写语义的请求使用 Action 契约。实现不得自行把 `params` 改写为 body。
...
### 5.2 requestInterceptor 的 GET-only 边界

宿主 `requestInterceptor` 可以注入 header、credentials、abort signal 和超时元数据，但不得改变协议已确定的 method、query 或 body 语义。DataRef 与 optionsSource 的 GET 请求在拦截器返回后仍必须没有 body；若拦截器试图增加 body 或改成其他 method，Renderer 必须返回 `INTERCEPTOR_VIOLATION`，不得发出修改后的请求。
...
| 情形 | Renderer 行为 |
|---|---|
| 请求超时（> `requestTimeout` 配置，默认 10s） | 节点进入错误态，展示"请求超时，请稍后重试"；Action 请求结果为 `unknown`，是否重试由 Action 的 `retryPolicy` 决定 |
| 网络中断（`fetch` 抛出 `TypeError: Failed to fetch`） | 节点进入错误态，展示"网络异常，请检查网络连接"；Action 请求结果为 `unknown`，是否重试由 Action 的 `retryPolicy` 决定 |
...
`source: static` 直接提供同步 `value`，不产生网络请求、loading、timeout 或 network error。`source: ref` 指向 static datasource 时继承同一同步语义；不得声明 `responseMapping`。

- `table` / `chart` 的 static/ref `value` 必须是数组；数组项由组件字段契约读取。
- `statCard` / `text` 的 static/ref `value` 可以是标量或 JSON object；声明 `props.valueField` 时必须从 object 的 own-property 取值。
- 静态值形状不符合组件契约时，Renderer 返回 `STATIC_DATA_SHAPE_MISMATCH`，不得发起网络请求或静默转换。
...
若 `searchable: true`，前端会额外附加标准查询参数 `keyword`；`optionsSource.params` 不得声明 `keyword`。非空搜索词最后覆盖静态 params，空搜索词按 ADR-0010 tombstone 删除 `keyword`。
