...
当前协议版本：`v0.2`（最新补丁版本 `v0.2.2`，见 [CHANGELOG.md](./CHANGELOG.md)）。`meta.protocolVersion` 仅声明 MAJOR.MINOR（即 `"0.2"`），不含 PATCH 号——因此 `v0.2.0`、`v0.2.1` 与 `v0.2.2` 共享同一 `protocolVersion` 值。
