...
| [16-v2.5-release-goals.md](./16-v2.5-release-goals.md) | 维护者 | `v2.5` accept / 发布门禁（ADR-0025/0026/0027） |
...
- 维护或发布 `v2.5.x` → 对照 `16-v2.5-release-goals.md`；历史 `v2.4.x` → `15-v2.4-release-goals.md`；`14` / `13` / `12` / `10` / `09` 为历史发布证据。
- 当前 `v2.5` 制品 → 对照 `16-v2.5-release-goals.md`（G0–G5 已闭合）。
...
当前协议版本：`v2.5.0`，页面通过 `meta.protocolVersion: "2.5"` 声明 MAJOR.MINOR。`1.0` 页面不得直接进入 v2 标准 Renderer；必须继续由 v1 Renderer 消费，或由调用方显式执行迁移 adapter 后再交给 v2。标准 Renderer 入口不做版本猜测。v2.5 发布门禁见 [16-v2.5-release-goals.md](./16-v2.5-release-goals.md)；从 2.4 升级见 [migrations/2.4-to-2.5.md](./migrations/2.4-to-2.5.md)；v2.4 见 [15-v2.4-release-goals.md](./15-v2.4-release-goals.md) 与 [migrations/2.3-to-2.4.md](./migrations/2.3-to-2.4.md)；从 2.2 升级见 [migrations/2.2-to-2.3.md](./migrations/2.2-to-2.3.md)。
