...
- [ ] 不支持的 MAJOR 版本必须拒绝渲染；
- [ ] 同一 MAJOR 下，只有 Renderer 显式声明支持的 MINOR 才可加载；
- [ ] 删除“按最接近的已知版本尝试解析”的规则；
- [ ] 将缺失 `protocolVersion` 的 `v0.1` 页面限定到显式 legacy adapter；
- [ ] 增加支持版本、未知 MINOR、未知 MAJOR、缺失版本的正反例测试。

**验收：** 规范、Renderer 参考行为和一致性测试对每个输入给出唯一的接受或拒绝结果。
...
1. RC 保守方案：`data.params`、`datasources.*.params`、`optionsSource.params` 和 `requestMapping.query` 的最终值仅允许标量；
2. 扩展方案：保留数组和对象，并规定唯一的键展开、数组、`null`、空值及百分号编码算法。

无论选择哪一种，都必须完成：

- [ ] 定义 string / number / boolean / `null` / `undefined` 的编码或省略规则；
- [ ] 定义已有 URL query 与协议参数的合并规则；
- [ ] 定义重复 key、参数顺序和 UTF-8 百分号编码规则；
- [ ] 对 DataRef、远程选项和行级 Action 使用同一套公共算法；
- [ ] 提供输入对象到最终 URL 的字节级测试向量。

**验收：** JavaScript 参考实现与至少一个后端语言实现对全部测试向量产生相同结果。

### G3. 保留请求参数与冲突优先级

**目标：** 搜索字段、静态 params 与 Renderer 自动分页参数不会互相覆盖出不同结果。

- [ ] 明确 `page`、`pageSize`、`sort` 是否为保留参数；
- [ ] 明确搜索字段、`data.params`、当前分页/排序状态三者的合并优先级；
- [ ] 对冲突配置增加 L2 静态校验，或定义唯一覆盖结果；
- [ ] 增加搜索提交、翻页、排序及清空筛选的请求测试向量。

**验收：** 同一页面状态只能构造出一个确定的请求参数集合。
...
- [ ] 页面版本与 capability 协商；
- [ ] DataRef 请求构造、参数合并和 `responseMapping`；
- [ ] 搜索表单提交、服务端分页和排序；
- [ ] 表达式快照、批量提交、冲突及循环保护；
- [ ] 普通 Action、行级 Action 和 OutcomeBehavior 时序；
- [ ] `400` 字段错误、`401`/`403` hook、`404`、`5xx`、超时和中断；
- [ ] 单文件与多文件上传请求和响应取值；
- [ ] 六个官方场景的端到端期望结果。
