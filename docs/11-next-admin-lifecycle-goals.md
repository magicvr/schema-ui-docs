...
applies_to: schema-ui-protocol v2.4 (post-v2.3 track; Phase D partial)
---
...
以下为协议层仍**可能**升格为一等能力的候选；**当前不立项、不写 Schema、不进 conformance 正例**。实现方可继续用 Host Extension，但不得伪装为协议已支持：

| 候选 | 业务问题 | 建议触发条件 |
|---|---|---|
| 导入 / 导出向导 | 批量导入校验、导出当前筛选结果 | 多实现要对齐向导步骤与失败回显 |
| 异步任务中心 | 长任务提交、进度、结果回跳 | 出现跨页可声明的任务状态契约需求 |
| 树表 / 可展开行 | 层级数据、父行展开子表 | 需要标准 `$parentRow` 与嵌套挂载结构 |
| 行内编辑 | 单元格直接改值提交 | 高频且与 form 生命周期边界清晰 |
| 跨页全选当前筛选全集 | 选中不限当前页 | 与 ADR-0022 清空/键模型冲突需新 ADR |
| 批量部分成功 | 行级成功/失败回填 | 与整批一次 HTTP 语义冲突需新模型 |
| 详情内嵌操作条 | recordView 内 toolbar | 先观察同页 `actionButton` 是否足够 |

门禁原则不变：每项须独立 ADR（MVP + 非目标 + 失败策略）→ Schema/L2/fixtures 原子交付 → MINOR + capability。
...
4. `2.4` — Phase D.0：标准只读详情 `recordView`（ADR-0024，已发布）。
5. 更后 MINOR — Phase D.1 按真实需求单独立项。
...
本轨道的 **P0 与 P1（选择/批量、权限继承）** 已达成。P2 中 **标准只读详情** 已随 v2.4 交付；其余 P2 运营增强仍不纳入阻断条件，按 D.1 单独立项。
