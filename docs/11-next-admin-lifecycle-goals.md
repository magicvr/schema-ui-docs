...
| **P1** | 表格选择与批量操作 | 多选、批量确认、批量 request、成功刷新 | [ADR-0022](./decisions/0022-table-selection-and-batch-request.md)（**proposed**）· `table.selection`、`actions.batch.request` |
| **P1** | 权限继承补齐 | 容器级 edit/delete 等与子节点/操作级联 | 待起草 · **独立 ADR**（不并入 0022） |
...
- [ ] 官方扩展示例场景 + `2.1→2.2` 迁移短文 + `2.2.0` 制品打包。
...
3. **权限继承** — **不在 0022**；另开 ADR。

门禁额外要求：

- [ ] 与搜索分页排序状态机（ADR-0011）的交互有 fixtures；
- [ ] `table.selection` 与 `actions.batch.request` 可独立声明，互不隐式耦合。
...
2. `2.2` — Phase C：表格选择 + 批量 request（ADR-0022）；权限继承可同发或随后。
...
| 本轨道 | **Phase C 语义已落地**（ADR-0022 accepted + Schema/L2/fixtures）；`2.2.0` 制品打包待办 |
...
| 下一步具体动作 | 场景示例 + 迁移 + 择机 `2.2.0` 发布；权限继承 ADR |
